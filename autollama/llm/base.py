...
class ChatSequence:
    ...
    def for_model(cls, model_name: str, messages: list[Message] | ChatSequence = []):
        from autollama.llm.providers.groq import GROQ_CHAT_MODELS

        if not model_name in GROQ_CHAT_MODELS:
            raise ValueError(f"Unknown chat model '{model_name}'")

        return ChatSequence(
            model=GROQ_CHAT_MODELS[model_name], messages=list(messages)
        )
    ...
    def token_length(self):
        from autollama.llm.utils import count_message_tokens
